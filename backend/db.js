...
function migrate() {
  ...
  query(`INSERT INTO roles (name, description) VALUES ('administrador', 'Acceso completo') ON CONFLICT(name) DO NOTHING`);
  query(`INSERT INTO roles (name, description) VALUES ('doctor', 'Gestiona historias y odontograma') ON CONFLICT(name) DO NOTHING`);
  query(`INSERT INTO roles (name, description) VALUES ('invitado', 'Acceso limitado de consulta') ON CONFLICT(name) DO NOTHING`);
